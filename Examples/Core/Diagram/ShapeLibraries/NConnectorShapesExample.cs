...
namespace Nevron.Nov.Examples.Diagram
{
    public class NConnectorShapesExample : NExampleBase
    {
        ...
        {
            return @"<p>This example shows the connector shapes created by the ""NConnectorShapeFactory"".</p>";
        }
        ...
        private void InitDiagram(NDrawingDocument drawingDocument)
        {
			...
			connectorShapes.DefaultSize = new NSize(120, 90);

            int row = 0, col = 0;
            double cellWidth = 300;
            double cellHeight = 200;

            for (int i = 0; i < connectorShapes.ShapeCount; i++, col++)
            {
                NShape shape = connectorShapes.CreateShape(i);
