...
";
		...
		{
			NShape shape = m_BasicShapes.CreateShape(ENBasicShape.Rectangle);
		...
		{
			NShape shape = m_BasicShapes.CreateShape(ENBasicShape.Ellipse);
			shape.SetBounds(bounds);
			shape.UserClass = userClass;
			m_DrawingView.ActivePage.Items.Add(shape);
			return shape;
		...
		NBasicShapeFactory m_BasicShapes = new NBasicShapeFactory();
